...
main = Blueprint('SaldoClients_blueprint', __name__)
    ...
    try:
        ...
        sql_base = """
            SELECT 
                ...
                SUM(tr_cli.IMPORTE) - SUM(tr_cli.ABONO) AS Saldo
            FROM transaccion_clientes tr_cli
            JOIN clientes Cli ON tr_cli.clientes_id = Cli.id
            WHERE 1 = 1
        """
        params = []
        ...
        sql_base += " GROUP BY Cli.id, Cliente ORDER BY Saldo DESC"
        ...
        saldo_clientes = cursor.fetchall()
